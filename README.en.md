...
For “how much will I have to pay in card bills next month?”, use `financeiro_cartoes.faturas_a_vencer_no_periodo.total_por_moeda`. It adds the `totalAmount` of Credit Card Bills whose due date falls within the requested period. Treat the returned amount as complete only when `coverage.complete` is `true`; `saldo_centavos` is current card usage, not a bill. Coverage counts cards with a bill in the requested period and is complete only when every card has one. `bill_coverage_by_card` distinguishes bills found, bills outside the period, an empty response, and an API failure. An empty list or bills only outside the period does not prove that no bill is due that month.
