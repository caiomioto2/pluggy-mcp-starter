...
export type RefreshResult = {
  ...
  requires_reconnection: boolean; retryable: boolean; reason: string | null; next_allowed_at?: string;
};
...
export function itemStatus(itemId: string, payload: unknown): RefreshResult {
  ...
  return { item_id:itemId, refresh_requested:false, status, execution_status:executionStatus,
    ...
    requires_user_action:isUserAction(status,code), requires_reconnection:isReconnect(status,code), retryable:status === "UPDATING", reason:code };
}
...
function errorResult(itemId: string, error: unknown): RefreshResult {
  ...
  const result: RefreshResult = { item_id:itemId, refresh_requested:false, status:status === 409 ? "UPDATING" : null,
    ...
    reason:rateLimited ? "RATE_LIMIT" : code ?? (status === 404 ? "ITEM_NOT_FOUND" : status === 409 ? "CONFLICT" : "REFRESH_FAILED") };
  if (nextAllowedAt) result.next_allowed_at = nextAllowedAt;
  return result;
}

const sleep = (ms: number) => new Promise<void>(resolve => setTimeout(resolve, ms));
const forbidden = (itemId: string): RefreshResult => ({item_id:itemId,refresh_requested:false,status:null,execution_status:null,last_updated_at:null,completed:false,requires_user_action:false,requires_reconnection:false,retryable:false,reason:"ITEM_NOT_ALLOWED"});
